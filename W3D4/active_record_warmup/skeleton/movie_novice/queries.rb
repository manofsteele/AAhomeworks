...
def below_average_years
  ...
  Movie.where('score < ?', 5).group('yr').select('yr', 'COUNT(*) as bad_movies').order('bad_movies DESC')
end
...
def uma_movies
  ...
  Movie.joins(:actors).where('actors.name = ?', "Uma Thurman").select('id', 'title', 'yr').order('yr ASC')
end
