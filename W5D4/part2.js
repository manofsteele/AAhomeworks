function printCallback(el) {
  console.log(el);
}

function titleize(arr, callback) {
  result = arr.map(el => {
    return `Mx. ${el} Jingleheimer Schmidt`;
  });

  result.forEach(el => callback(el));

}

titleize(["Mary", "Brian", "Leo"], printCallback);

function Elephant(name, height, tricks = []) {
  this.name = name;
  this.height = height;
  this.tricks = tricks;

}

Elephant.prototype.trumpet = function() {
  console.log(`${this.name} the elephant goes 'phrRRRRRRRRRRR!!!!!!!'`);
}

Elephant.prototype.grow = function() {
  this.height += 12;
}

Elephant.prototype.addTrick = function(trick) {
  this.tricks.push(trick);
}

Elephant.prototype.play = function() {
  let trick = this.tricks[Math.floor(Math.random() * (this.tricks.length))];
  console.log(`${this.name} is ${trick}!`);
}

let dumbo = new Elephant("dumbo", 24);
dumbo.trumpet();
dumbo.grow();
console.log(dumbo.height);
dumbo.addTrick("painting a picture");
dumbo.play();

let ellie = new Elephant("Ellie", 185, ["giving human friends a ride", "playing hide and seek"]);
let charlie = new Elephant("Charlie", 200, ["painting pictures", "spraying water for a slip and slide"]);
let kate = new Elephant("Kate", 234, ["writing letters", "stealing peanuts"]);
let micah = new Elephant("Micah", 143, ["trotting", "playing tic tac toe", "doing elephant ballet"]);

let herd = [ellie, charlie, kate, micah];

Elephant.prototype.paradeHelper = function(eleph) {
  console.log(`${eleph.name} is trotting by!`);
}

herd.forEach(eleph => eleph.paradeHelper(eleph));


function dinerBreakfast(newfood = "") {
  this.order = "I'd like cheesy scrambled eggs please.";
  return function (newfood) {
    this.order = this.order.slice(0, this.order.length - 8) + ` and ${newfood} please`;
    console.log(this.order);
  };
}

let bfastOrder = dinerBreakfast();
bfastOrder("chocolate chip pancakes");
bfastOrder("grits");
